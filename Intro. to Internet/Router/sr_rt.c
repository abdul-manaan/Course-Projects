...
struct sr_rt* getRoutingTableEntry(struct sr_instance* sr, uint32_t dip){
    struct sr_rt* eachEntry = sr->routing_table;
    struct sr_rt* result = NULL;
    uint32_t longestIP = INT_MAX;
    struct in_addr longIP;
    longIP.s_addr = dip;
    while(eachEntry){
        if(longIP.s_addr  == eachEntry->dest.s_addr ){
            return eachEntry;
        }
